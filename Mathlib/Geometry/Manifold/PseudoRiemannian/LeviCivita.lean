...
Existence of a Levi-Civita connection is still Riemannian-only, in
`Mathlib.Geometry.Manifold.VectorBundle.CovariantDerivative.LeviCivita`: the Koszul construction
recovers `∇_X Y` from `⟪∇_X Y, Z⟫` by `inner_self_eq_zero`, which fails for null vectors.
...
private lemma diffAt_antisymm (hm : cov.IsMetricCompatible)
    ...
    pseudoInner (diffAt cov cov' x u v) w = -pseudoInner (diffAt cov cov' x w v) u := by
  have hu := mdifferentiableAt_extend I E u
  ...
  have key : pseudoInner (diffAt cov cov' x u v) w + pseudoInner u (diffAt cov cov' x w v) = 0 := by
    simp only [hu', hw', sub_apply, pseudoInner_sub_left, pseudoInner_sub_right]
    linarith
  rw [pseudoInner_comm u (diffAt cov cov' x w v)] at key
  linarith
